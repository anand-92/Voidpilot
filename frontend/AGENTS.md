...
*   **`brainstorm/`**: Components exclusively used in the Brainstorm mode workspace (e.g., canvas elements, artifact renderers, specific panels).
    *   `BrainstormControls.tsx`: The chat input and connection controls. Includes tool selection toggles (Artifact, Image, Video) that can be enabled before starting a session.
    *   `BrainstormLayouts.tsx`: Defines `BrainstormLayoutProps` type used by both desktop and mobile layouts.
...
*   `useGeminiBrainstorm.ts`: Transport and state for the Brainstorm mode websocket. Exports `BRAINSTORM_TOOL_OPTIONS` for tool selection and `BRAINSTORM_FLASH_MODEL_OPTIONS` for model selection.
